  ...
  data: new SlashCommandBuilder()
    ...
    .addRoleOption((role) =>
      role
        .setName("role")
        .setDescription(
          "The role to pick the winner from. To pick from all members, use @everyone."
        )
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (!interaction.guild.available) {
      return await interaction.reply({
        content:
          "Sorry, the server you're in seems to be unavailable. There seems to be a server outage.",
      });
    }

    await interaction.reply({
      content: `The winner of "${subject}" is ${winner}!`,
    });
