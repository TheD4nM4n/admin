...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("enable")
        .setDescription("Enables chat filtering in this server.")
    )
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("disable")
        .setDescription("Disables chat filtering in this server.")
    )
    ...
    .addSubcommandGroup((subcommandGroup) =>
      subcommandGroup
        ...
        .addSubcommand((subcommand) =>
          subcommand
            .setName("enable")
            .setDescription("Enables infraction logging for the server.")
        )
        ...
        .addSubcommand((subcommand) =>
          subcommand
            .setName("disable")
            .setDescription("Disables infraction logging for the server.")
        )
    )
    ...
    .addSubcommandGroup((subcommandGroup) =>
      subcommandGroup
        .setName("list")
        .setDescription(
          "Changes settings for the list of words banned by Admin."
        )
        ...
        .addSubcommand((subcommand) =>
          subcommand
            .setName("add")
            .setDescription(
              "Adds the defined word to the list of banned words for the server."
            )
            .addStringOption((builder) =>
              builder
                .setName("word")
                .setDescription("Word to add to the list of banned words.")
                .setRequired(true)
            )
        )
        ...
        .addSubcommand((subcommand) =>
          subcommand
            ...
            .addStringOption((builder) =>
              builder
                .setName("word")
                .setDescription("Word to remove from the list of banned words.")
                .setRequired(true)
            )
        )
        ...
        .addSubcommand((subcommand) =>
          subcommand
            .setName("default")
            .setDescription(
              "Allows enabling or disabling of the built-in list of banned words."
            )
            .addStringOption((option) =>
              option
                .setName("state")
                .setDescription(
                  "The state you would like the built-in profanity list to use."
                )
                .setRequired(true)
  ...
  async execute(interaction) {
    if (interaction.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
      ...
      if (
        interaction.options.getSubcommandGroup((required = false)) === "log"
      ) {
        switch (interaction.options.getSubcommand()) {
          case "channel":
            const channel = interaction.options.getChannel("log-channel");
            config[`${interaction.guildId}`]["chat-filter"]["log-channel"] =
              channel.id;
            fs.writeFile(
        ...
        interaction.options.getSubcommandGroup((required = false)) === "list"
      ) {
        switch (interaction.options.getSubcommand()) {
          case "add":
            const wordToAdd = interaction.options.getString("word");
            config[`${interaction.guildId}`]["chat-filter"][
              "custom-words"
            ].push(wordToAdd);
            fs.writeFile(
              "./data/guildConfig.json",
          ...
          case "remove":
            const wordList =
              config[`${interaction.guildId}`]["chat-filter"]["custom-words"];
